...
The five open-corpus arms are all conditioned on the acquisition covariates
(collision energy, adduct, polarity, instrument). In earlier versions of this work
M0 alone was not: its encoder accepted the covariate vector and discarded it, so the
binned baseline was the only arm predicting without knowing how the spectrum had
been acquired. The M0 rows above are refits under the corrected baseline and
supersede the figures reported previously; the main text states what the correction
cost the headline claim. The three single-library M0 rows are the original,
un-reconditioned runs and carry the smaller parameter count ($1{,}164{,}560$ against
the corrected baseline's $1{,}172{,}752$) that marks them as such. They are retained because the
single-library ladder is used in the main text only for the mass-axis and
aggregation contrasts, neither of which involves M0; the encoder and interface
contrasts are reported for the open corpus alone.
...
which has no attention arm, the composite encoder effect is shown instead, and all
three distributions overlap --- the mass-axis and aggregation ones heavily, which is
the same fact Table~\ref{tab:s3} records as a sign reversal.}
\label{fig:s1}
\end{figure}
