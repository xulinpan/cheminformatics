...
($+0.0028$, $+0.0045$, $+0.0037$). That ranking we regard as established.
...
magnitude smaller than either effect. Against the unconditioned baseline the encoder
...
The mass-axis effect falls from $+0.0268$ on the single-library corpus to $+0.0142$
on the open one, roughly halving as the instrument mix widens from one to $66$.
Aggregation falls comparably, from $+0.0322$ to $+0.0179$. Both comparisons are
between arms that differ from each other in the same way on both corpora, so both
are sound.

The encoder effect is not yet comparable across the two. The single-library arms
were fitted before the baseline was given the acquisition covariates, so its
$+0.0377$ is measured against an unconditioned M0 and the open corpus's $+0.0149$
against a conditioned one. Rerunning M0 on the single-library corpus is the obvious
remedy and we report the two encoder figures as not comparable until it is done. The
mass-axis and aggregation rows in Table~\ref{tab:table3_two_corpora} are unaffected,
because neither contrast involves M0.
