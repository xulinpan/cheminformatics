...
DESC = {"M0":  ("0.5 Da bins", "dense vector, MLP", "mean", "1,172,752"),
        ...
        "M2":  ("full precision", "peak tokens, attention", "hierarchical", "829,584")}
t1 = pd.DataFrame([{
...
} for r in ["M0", "M1D", "M1R", "M1", "M2"]])
F.write_table(t1, out, "table1_arms_open",
              f"The five arms on the open corpus, each differing from the one above it "
...
NAME = {"M1D-M0": "interface", "M1R-M1D": "attention", "M1R-M0": "encoder",
        "M1-M1R": "mass axis", "M2-M1": "aggregation",
...
} for k in ["M1D-M0", "M1R-M1D", "M1R-M0", "M1-M1R", "M2-M1", "M1-M0", "M2-M0"]])
F.write_table(t2, out, "table2_effects_open",
              ...
              "The interface and attention rows decompose the encoder row; the attention "
              "interval includes zero. The final column is the standard deviation of the "
              "effect across seeds.")
